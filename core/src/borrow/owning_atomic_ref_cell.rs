...
pub struct OwningAtomicRefMutGuard<'a, T, U> {
    owner: Arc<AtomicRefCell<T>>,
    guard: Option<AtomicRefMut<'a, U>>,
}

impl<'a, T, U> Drop for OwningAtomicRefMutGuard<'a, T, U> {
    fn drop(&mut self) {
...
impl<'a, T, U> OwningAtomicRefMutGuard<'a, T, U> {
    ...
    pub fn map<Q, F: FnOnce(&mut U) -> &mut Q>(self, map: F) -> OwningAtomicRefMutGuard<'a, T, Q> {
        let (owner, guard) = self.into_raw();
        let guard = AtomicRefMut::map(guard, map);
        OwningAtomicRefMutGuard::from_raw(owner, guard)
    }

    pub fn fiter_map<Q, F: FnOnce(&mut U) -> Option<&mut Q>>(
        self,
        map: F,
    ...
    pub fn try_map<Q, E, F: FnOnce(&mut U) -> Result<&mut Q, E>>(
        self,
        map: F,
...
impl<'a, T, U> Deref for OwningAtomicRefMutGuard<'a, T, U> {
    type Target = U;
...
impl<'a, T, U> DerefMut for OwningAtomicRefMutGuard<'a, T, U> {
...
pub struct OwningAtomicRefGuard<'a, T, U> {
    owner: Arc<AtomicRefCell<T>>,
    guard: Option<AtomicRef<'a, U>>,
}

impl<'a, T, U> Drop for OwningAtomicRefGuard<'a, T, U> {
    fn drop(&mut self) {
        // make sure the guard is dropped before the owner to avoid any use after free
        self.guard = None;
    }
...
impl<'a, T, U> OwningAtomicRefGuard<'a, T, U> {
    ...
    pub fn map<Q, F: FnOnce(&U) -> &Q>(self, map: F) -> OwningAtomicRefGuard<'a, T, Q> {
        let (owner, guard) = self.into_raw();
        let guard = AtomicRef::map(guard, map);
        OwningAtomicRefGuard::from_raw(owner, guard)
    }

    pub fn fiter_map<Q, F: FnOnce(&U) -> Option<&Q>>(self, map: F) -> Option<OwningAtomicRefGuard<'a, T, Q>> {
        let (owner, guard) = self.into_raw();
        let guard = AtomicRef::filter_map(guard, map)?;
        Some(OwningAtomicRefGuard::from_raw(owner, guard))
    }

    pub fn try_map<Q, E, F: FnOnce(&U) -> Result<&Q, E>>(self, map: F) -> Result<OwningAtomicRefGuard<'a, T, Q>, E> {
        let (owner, guard) = self.into_raw();
        // Atomic ref has no try_map, thus store the result in a temporary.
        let mut error = None;
        let guard = AtomicRef::filter_map(guard, |a| match map(a) {
            Err(err) => {
                error = Some(err);
                None
            }
...
impl<'a, T, U> Deref for OwningAtomicRefGuard<'a, T, U> {
    type Target = U;
